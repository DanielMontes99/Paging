...
class LoginController {
    ...
    public function userRegister($datos){
        $user = new user;
        $result = $user->whereor([['user_Email',$datos['user_Email']],['user_Alias',$datos['user_Alias']]])
                       ->get();
        if(sizeof(json_decode($result)) > 0){
            echo json_encode(["r"=>false]);
        }else{
            $result1 = $user->into([['user_Email'],['user_Alias'],['user_Pass']])
                            ->values([[$datos['user_Email']],[$datos['user_Alias']],[$datos['user_Pass']]])
                            ->insert();
    ...
    public function userAuth($datos){
        $user = new user;
        $result = $user->where([['user_Email',$datos['user_Email']],['user_Pass',$datos['user_Pass']]])
                       ->get();
        if(sizeof(json_decode($result)) > 0){
            return $this->sessionRegister($datos);
        }else{
